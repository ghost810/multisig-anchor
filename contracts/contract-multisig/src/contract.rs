...
use cosmwasm_std::{
    ...
    Response, StdResult, WasmMsg, coin, QueryRequest, BankQuery, BalanceResponse,
...
use moneymarket::market::{HandleMsg, Cw20HookMsg};
// use anchor_address::
use crate::error::ContractError;
use crate::msg::{ExecuteMsg, InstantiateMsg, QueryMsg};
use crate::state::{
    next_id, parse_id, Ballot, Config, Proposal, BALLOTS, CONFIG, PROPOSALS, VOTERS, PRINCIPLE_AMOUNT//, load_token_balance
};

// version info for migration info
const CONTRACT_NAME: &str = "contract-multisig";
...
pub fn instantiate(
    ...
    if _info.funds.is_empty(){
        return Err(ContractError::ZeroUSD {});
    }

    let index = _info.funds.iter()
    .enumerate()
    .find_map(|(i, exist)| {
        if exist.denom == "uusd" {
            Some(i)
    ...
    match index {
        Some(id) => {
            let am = _info.funds[id].amount.clone();
            PRINCIPLE_AMOUNT.save(deps.storage, &am)?;
        }
        None => {
            return Err(ContractError::ZeroUSD{});
        }
    ...
    let msg: CosmosMsg = CosmosMsg::Wasm(WasmMsg::Execute {
        contract_addr: CONFIG.load(deps.storage)?.market_address,
        funds: vec![coin(u128::from(PRINCIPLE_AMOUNT.load(deps.storage)?), "uusd")],
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::SendYield { amount } => execute_send_yield(deps, info, amount)
    }
}

pub fn execute_send_yield (
    deps: DepsMut,
    info: MessageInfo,
    amount: Uint128,
) -> Result<Response<Empty>, ContractError> {
    // withdraw from anchor token
    let aust_balance: BalanceResponse = deps.querier.query(&QueryRequest::Bank(BankQuery::Balance {
        address: info.sender.to_string(),
        denom: "ausd".to_string(),
    }))?;

    println!("amount {}, {}, {}", amount, aust_balance.amount.amount, Uint128::from(PRINCIPLE_AMOUNT.load(deps.storage)?));
    
    if aust_balance.amount.amount < PRINCIPLE_AMOUNT.load(deps.storage)? + amount 
    {
        return Err(ContractError::InsufficientAmount {});
    }
   
    let current_usd: BalanceResponse = deps.querier.query(&QueryRequest:: Bank(BankQuery::Balance{
        address: info.sender.to_string(),
        denom: "uusd".to_string(),
    }))?;
    let yield_usd = current_usd.amount.amount + amount;
    // println!("------ yield amount -----------{}", yield_usd);
    
    let msgs: Vec<CosmosMsg> = vec![CosmosMsg::Wasm(WasmMsg::Execute {
        contract_addr: CONFIG.load(deps.storage)?.anchor_address,
        funds: vec![],
        msg: to_binary(&Cw20ExecuteMsg::Send {
            contract: CONFIG.load(deps.storage)?.market_address,
            amount: amount,
    ...
    CosmosMsg::Bank(BankMsg::Send {
        to_address: CONFIG.load(deps.storage)?.recipient,
        amount: vec![
            Coin {
                denom: "uusd".to_string(),
                amount: yield_usd,
    ...
    Ok(
        Response::new()
        .add_messages(msgs)
        .add_attribute("action", "send_yield")
    )
}
...
pub fn execute_propose(
    ...
    // }))?;
    // println!("--------------- balance ---------{}", balance.amount.amount.to_string());
    let prop = Proposal {
        title,
        description,
        expires,
...
pub fn query(deps: Deps, env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        QueryMsg::PrincipleAmount {} => to_binary(&query_principle_amount(deps)?),
...
fn query_principle_amount(deps: Deps) -> StdResult<Uint128> {
    let amount = PRINCIPLE_AMOUNT.load(deps.storage)?;
    Ok(amount)
}
...
mod tests {
    ...
    const MARKET: &str = "amarket";
    const TOADDRESS: &str = "toher";
    ...
    fn setup_test_case(
        ...
        let instantiate_msg = InstantiateMsg {
            anchor_address: ANCHOR.into(),
            market_address: MARKET.into(),
            recipient: TOADDRESS.into(),
            voters,
            required_weight,
            max_voting_period,
        };
    ...
    fn test_instantiate_works() {
        ...
        let err =
            instantiate(deps.as_mut(), mock_env(), info.clone(), instantiate_msg.clone()).unwrap_err();
        ...
        let err =
            instantiate(deps.as_mut(), mock_env(), info.clone(), instantiate_msg).unwrap_err();
        assert_eq!(err, ContractError::ZeroUSD {});

        let info = mock_info(OWNER, &[coin(100, "ukrw"), coin(100_000_000, "uusd"), coin(100, "uluna")]);
        ...
        let res = setup_test_case(deps.as_mut(), info, required_weight, max_voting_period).unwrap();
        assert_eq!(
            res.messages[0].msg,
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: MARKET.to_string(),
                funds: vec![coin(100_000_000 as u128, "uusd")],
                msg: to_binary(&HandleMsg::DepositStable {
                })
                .unwrap()
            })
    ...
    fn test_propose_works() {
        ...
        let info = mock_info(OWNER, &[coin(100_000_000, "uusd"), coin(100, "uluna")]);
        ...
        let info = mock_info(SOMEBODY, &[coin(100_000_000, "uusd"), coin(100, "uluna")]);
        ...
        let info = mock_info(VOTER3, &[coin(100_000_000, "uusd"), coin(100, "uluna")]);
        ...
        let info = mock_info(VOTER4, &[coin(100_000_000, "uusd"), coin(100, "uluna")]);
    ...
    fn test_vote_works() {
        ...
        // But voter1 can
        let info = mock_info(VOTER1, &[coin(100_000_000, "uusd"), coin(100, "uluna")]);
        ...
        let info = mock_info(VOTER2, &[coin(100_000_000, "uusd"), coin(100, "uluna")]);
        ...
        let info = mock_info(VOTER5, &[coin(100_000_000, "uusd"), coin(100, "uluna")]);
    ...
    fn test_yield_works() {
        ...
        let info = mock_info(OWNER, &[coin(0, "uusd"), coin(100, "uluna")]);
        setup_test_case(deps.as_mut(), info.clone(), required_weight, voting_period).unwrap();
        ...
        let send_yield = ExecuteMsg::SendYield{
            amount,
        };
        let res = execute(
            deps.as_mut(),
        ...
        .unwrap();
        let action: String = res.attributes[0].value.parse().unwrap();
        assert_eq!(action, "send_yield");
        assert_eq!(
            res.messages[0].msg,
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: ANCHOR.into(),
                funds: vec![],
                msg: to_binary(&Cw20ExecuteMsg::Send {
